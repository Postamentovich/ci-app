import gitRepo from '../../../server/utils/git-repo';
import { storageAPI } from '../../../server/api/storage-api';
...
describe('Взаимодействие с локальным репозиторием', () => {
  ...
  test('Клонирование репозитория', async () => {
    ...
    expect(gitRepo.run).toHaveBeenCalledWith(`git clone https://github.com/repoName folderName`);
  });
});
